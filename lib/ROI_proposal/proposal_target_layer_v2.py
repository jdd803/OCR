...
def proposal_target_layer(rpn_rois, gt_boxes, _num_classes, offsets):
    '''
    Make Python version of _proposal_target_layer_py below Tensorflow compatible
    '''
    rois, preds, labels, bbox_targets, bbox_inside_weights, bbox_outside_weights = tf.py_func(_proposal_target_layer_py,
                                                                                       [rpn_rois, gt_boxes,
                                                                                        _num_classes, offsets],
                                                                                       [tf.float32, tf.int32,
                                                                                        tf.float32, tf.float32,
                                                                                        tf.float32, tf.float32])

    rois = tf.reshape(rois, [-1, 5], name='rois')
    preds = tf.reshape(preds, [-1,5], name='preds')
    labels = tf.convert_to_tensor(tf.cast(labels, tf.int32), name='labels')
    bbox_targets = tf.convert_to_tensor(bbox_targets, name='bbox_targets')
    bbox_inside_weights = tf.convert_to_tensor(bbox_inside_weights, name='bbox_inside_weights')
    bbox_outside_weights = tf.convert_to_tensor(bbox_outside_weights, name='bbox_outside_weights')

    return rois, preds, labels, bbox_targets, bbox_inside_weights, bbox_outside_weights


def _proposal_target_layer_py(rpn_rois, gt_boxes, _num_classes, offsets):
    """
    ...
    # targets
    labels, rois, preds, bbox_targets, bbox_inside_weights = _sample_rois(
        all_rois, gt_boxes, fg_rois_per_image,
        rois_per_image, _num_classes, offsets)
    ...
    return np.float32(rois), np.float32(preds), labels, bbox_targets, bbox_inside_weights, bbox_outside_weights
...
def _sample_rois(all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes, offsets):
    ...
    return labels, rois, offset, bbox_targets, bbox_inside_weights